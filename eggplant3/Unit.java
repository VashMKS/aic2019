...
public class Unit {
    ...
    void reportMap() {
        // uc.println(uc.getType().toString() + " ID" + data.ID + " with vision range " + data.sightRange + "^2 = " + data.sightRangeSquared + " performing map update:");
        for (int i = -data.sightRange; i < 3; i++) {
            for (int j = -data.sightRange; j < data.sightRange; j++) {
                if (i*i + j*j < data.sightRangeSquared && i*i + j*j > data.sightRange*data.sightRange) {
    ...
    void reportLocation(Location loc) {
        ...
        // uc.println("  - content update complete: " + coord.content);
        ...
        data.map.updateMap(loc.x,loc.y,coord.content,data.currentRound,unit);
